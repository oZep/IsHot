        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key == pygame.K_LEFT or event.key == pygame.K_a: # referencing right and left arrow keys
                        self.movement[0] = True
                    if event.key == pygame.K_RIGHT or event.key == pygame.K_d: 
                        self.movement[1] = True
                    if event.key == pygame.K_UP or event.key == pygame.K_SPACE: # jump!, dont care about it's release as I dont want a constant jump on hold
                        if self.player.jump():  # velocity pointing upwards, gravity will pull player back down over time
                            self.sfx['jump'].play()
                    if event.key == pygame.K_x or event.key == pygame.K_e: # shoot
                        self.player.dash()
